...
class Activities extends EventEmitter {
  ...
  _run(activity) {
    ...
    const unsubscribe = busClient.events.subscribe(
      `${activityName}.${activity.msg.id}.(finished|error)`,
      unsub
    );

    const transport = busClient.events.connectedWith();
    if (activity.msg._xcraftTransports.indexOf(transport) === -1) {
      activity.msg._xcraftTransports.push(transport);
    }
