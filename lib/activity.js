...
class Activities extends EventEmitter {
  ...
  _run (activity) {
    ...
    const finishTopic = `${activityName}.${activity.msg.id}.finished`;
    const errorTopic = `${activityName}.${activity.msg.id}.error`;
    ...
    const unsubscribeFinished = busClient.events.subscribe (finishTopic, unsub);
    const unsubscribeError = busClient.events.subscribe (errorTopic, unsub);
