...
class Activities extends EventEmitter {
  ...
  _run (activity) {
    ...
    const realFinishHandler = busClient.getEventsRegistry ()[finishTopic];

    xLog.verb (`start new activity for ${activityName}`);

    busClient.events.send ('greathall::activity.started', activity);
    busClient.events.subscribe (finishTopic, msg => {
      busClient.events.unsubscribe (finishTopic);
      ...
      if (realFinishHandler) {
        xLog.verb (`calling finish handler`);
        realFinishHandler (msg);
      }
